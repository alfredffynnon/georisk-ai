...
export default function SignUpPage({ searchParams }: SignUpPageProps) {
  async function signUp(formData: FormData) {
    ...
    if (!email || !password) {
      redirectWithError("/auth/signup", "Email and password are required.");
    }
    ...
    if (errorMessage) {
      redirectWithError("/auth/signup", errorMessage);
    }
      ...
      <section className="w-full max-w-md rounded-md border border-white/10 bg-card p-8 shadow-2xl shadow-black/30">
        <div className="mb-8 flex flex-col gap-3">
          ...
          <h1 className="text-3xl font-semibold tracking-normal text-foreground">
            Sign up for GeoRisk AI
          </h1>
          <p className="text-sm leading-6 text-muted-foreground">
            Access the private geopolitical and climate risk workspace.
          </p>
        </div>
